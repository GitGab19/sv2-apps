...
/// correct behavior of handling CloseChannel messages.
///
/// We first send a CloseChannel message to a single channel, and assert that no shares are submitted from it.
/// Then we send a CloseChannel message to the group channel, and assert that no shares are submitted from any channel.
#[tokio::test]
