...
is_watched_config_file() {
  case "$1" in
    ...
    docker/config/jdc-config.toml.template) return 0 ;;
    docker/config/translator-proxy-config.toml.template) return 0 ;;
    *) return 1 ;;
  esac
}
...
git diff --name-only "$BEFORE_REF" "$AFTER_REF" -- \
  ...
  docker/config/jdc-config.toml.template \
  docker/config/translator-proxy-config.toml.template |
