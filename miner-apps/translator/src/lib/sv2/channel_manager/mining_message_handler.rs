...
impl HandleMiningMessagesFromServerAsync for ChannelManager {
        ...
        let success = self
            ...
            .super_safe_lock(|channel_manager_data| {
                ...
                if channel_manager_data.mode == ChannelMode::Aggregated {
                    ...
                    Ok::<OpenExtendedMiningChannelSuccess<'static>, TproxyError<error::ChannelManager>>(new_open_extended_mining_channel_success.into_static())
                } else {
                    ...
                    if m.extranonce_size as usize != downstream_extranonce_len {
                        ...
                        Ok::<OpenExtendedMiningChannelSuccess<'static>, TproxyError<error::ChannelManager>>(m.into_static())
                    }
                }
            })
            .map_err(|e| {
    ...
    async fn handle_close_channel(
        ...
        self.channel_manager_data
            .super_safe_lock(|channel_data_manager| {
                ...
                Ok::<(), TproxyError<error::ChannelManager>>(())
            })?;
        Ok(())
    }
    ...
    async fn handle_new_extended_mining_job(
        ...
        let new_extended_mining_job_messages_sv1_server = self
            .channel_manager_data
            .super_safe_lock(|channel_manage_data| {
                let mut new_extended_mining_job_messages = Vec::new();

                // was the message sent to a group channel?
                if let Some(group_channel_arc) =
                    channel_manage_data.group_channels.get(&m.channel_id)
                {
                    let mut group_channel = group_channel_arc.write().map_err(|e| {
                        error!("Failed to read group channel: {:?}", e);
                        TproxyError::shutdown(TproxyErrorKind::PoisonLock)
                    ...
                    // aggregated channel
                    if let Some(aggregated_channel) =
                        channel_manage_data.upstream_extended_channel.as_ref()
                    {
                        let mut aggregated_channel = aggregated_channel.write().map_err(|e| {
                            error!("Failed to read aggregated channel: {:?}", e);
                            TproxyError::shutdown(TproxyErrorKind::PoisonLock)
                        })?;
                        if group_channel
                            .get_channel_ids()
                            .contains(&aggregated_channel.get_channel_id())
                        {
                            ...
                            for (_, channel) in channel_manage_data.extended_channels.iter() {
                            ...
                            // exit the closure early, there's nothing else to do
                            return Ok::<
                                Vec<NewExtendedMiningJob<'static>>,
                                TproxyError<error::ChannelManager>,
                            >(new_extended_mining_job_messages);
                        }
                    }
                    ...
                    for channel_id in group_channel.get_channel_ids() {
                        let channel = channel_manage_data
                            .extended_channels
                            .get(channel_id)
                ...
                // if the message was not sent to a group channel, we need to check if we're
                // working in aggregated mode
                } else if channel_manage_data.mode == ChannelMode::Aggregated {
                    let mut aggregated_channel = channel_manage_data
                    ...
                    if aggregated_channel.get_channel_id() == m_static.channel_id {
                        ...
                        // we got a nonsense channel id, we should trigger fallback
                        error!("Channel not found: {}", m_static.channel_id);
                ...
                // mode, we need to process the message for a specific channel
                    ...
                    let channel = channel_manage_data
                        ...
                        .ok_or(TproxyError::fallback(TproxyErrorKind::ChannelNotFound))?; // we got a nonsense channel id, we should trigger fallback
                ...
                Ok::<Vec<NewExtendedMiningJob<'static>>, TproxyError<error::ChannelManager>>(
                    new_extended_mining_job_messages,
                )
            })?;
    ...
    async fn handle_set_new_prev_hash(
        ...
        let new_extended_mining_job_messages_sv1_server = self
            ...
            .super_safe_lock(|channel_manager_data| {
                    ...
                    channel_manager_data.group_channels.get(&m.channel_id)
                {
                    ...
                    // aggregated channel
                    if let Some(aggregated_channel) =
                        channel_manager_data.upstream_extended_channel.as_ref()
                    {
                        let mut aggregated_channel = aggregated_channel.write().map_err(|e| {
                            error!("Failed to read aggregated channel: {:?}", e);
                            TproxyError::shutdown(TproxyErrorKind::PoisonLock)
                            ...
                            .contains(&aggregated_channel.get_channel_id())
                        {
                            ...
                            let mut new_extended_mining_job_message = aggregated_channel
                                .get_active_job()
                                .ok_or(TproxyError::fallback(TproxyErrorKind::JobNotFound))?
                                .clone();
                ...
                // if the message was not sent to a group channel, we need to check if we're
                // working in aggregated mode
                } else if channel_manager_data.mode == ChannelMode::Aggregated {
                    let aggregated_channel_guard = channel_manager_data
                        .upstream_extended_channel
                        .as_ref()
                        .ok_or(TproxyError::fallback(TproxyErrorKind::ChannelNotFound))?;
                    let mut aggregated_channel = aggregated_channel_guard.write().map_err(|e| {
        ...
        // we need to send the only one SetNewPrevHash message to the SV1Server
        self.channel_state
            .sv1_server_sender
            .send((Mining::SetNewPrevHash(m_static), None))
            .await
            .map_err(|e| {
                ...
                .super_safe_lock(|channel_manager_data| {
                        ...
                        channel_manager_data.group_channels.get(&m.channel_id)
                    {
                            ...
                            channel_manager_data.upstream_extended_channel.as_ref()
                        {
                            ...
                            {
                                // update aggregated channel state
                                aggregated_channel.set_target(Target::from_le_bytes(
                                    m.maximum_target
                                        .inner_as_ref()
                                        .try_into()
                                ...
                                // for the aggregated channel, send one SetTarget message to the
                                // SV1Server
                    ...
                    // mode, we need to process the message for a specific channel
                        ...
                        let mut channel = channel_manager_data
                            .extended_channels
                            .get(&m.channel_id)
                            .ok_or(TproxyError::fallback(TproxyErrorKind::ChannelNotFound))? // we got a nonsense channel id, we should trigger fallback
                            .write()
                    ...
                    Ok::<Vec<SetTarget<'static>>, TproxyError<error::ChannelManager>>(
                        set_target_messages,
                    )
                })?;
